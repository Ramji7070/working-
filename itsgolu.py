...
from utils import progress_bar as ext_progress_bar
from vars import *
...
def decrypt_and_merge_video(mpd_url, keys_string, output_path, output_name, quality="720"):
    try:
        ...
        cmd4 = f'ffmpeg -y -i "{output_path}/video.mp4" -i "{output_path}/audio.m4a" -c copy "{output_path}/{output_name}.mp4"'
        os.system(cmd4)
...
async def download_video(url: str, output_name: str, quality: str = "480p"):
    ...
    cmd = (
        f'yt-dlp -f "{yt_format}" --merge-output-format mp4 '
        f'--external-downloader aria2c '
        f'--external-downloader-args "-x8 -s8 -j8 -k1M" '
        f'-o "{final_file}" "{url}"'
    )
...
async def safe_video_send(bot, delay=45, retries=5, **kwargs):
    attempt = 0
...
async def send_vid(
    ...
    try:
        ...
        # ===== LARGE FILE =====
            ...
            notify_split = await m.reply_text("⚠️ Splitting large video...")
            ...
            for idx, part in enumerate(parts):
                ...
                upload_msg = await m.reply_text(f"📤 Uploading Part {idx+1}...")
